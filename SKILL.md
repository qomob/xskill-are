...
根据 `capabilities` 列表自动匹配模板，当任一 capability 包含以下关键词时切换：

| 匹配关键词 | 选用模板 |
|-----------|---------|
| 营销/市场/品牌/广告/推广/销售/运营/社交媒体 | 营销类模板（默认） |
| 代码/编程/开发/架构/后端/前端/API/测试/部署/DevOps/安全/数据/算法/AI/机器学习 | 技术类模板 |

若同时匹配两类，优先使用匹配数更多的类别；若均不匹配且未提供 `task_brief`，默认使用营销类模板。
...
```
请基于你的专业能力（{capability}），帮我处理以下需求：
我的团队正在开发一个中型项目，需要在2周内完成一个技术可行性验证（PoC）。
当前有3名工程师可用，基础设施预算2万元。
请给出具体的技术方案和实施计划。
```
...
| 默认 Brief 有领域偏向 | Brief 模板支持按 capabilities 自动选择营销类/技术类；未匹配时默认使用营销类，对非两类领域的 Skill 公平性有限 |
...
2. **能力列表为空时业务维度无法评测** — `capabilities` 为空时，维度 1 得保守分 40，维度 5（复用维度 1 输出）也得 40；Brief 模板选择退化为默认营销类。
...
8. **Brief 模板可被显式覆盖** — 用户可以通过 `task_brief` 参数手动指定业务测试 Brief，绕过自动路由。这在需要针对非标准场景评测时有用，但也可能降低评测基准的一致性。
